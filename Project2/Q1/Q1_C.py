def main():
	...
	# X and Y separation
	train_X = data_training_np[:,0]
	train_Y = data_training_np[:,1]
	...
	lst_error = []                  # Empty list where I will append the errors
	for i in range(1, k+1):          # Loop over different values of k
		...
		for j in range(d+1):         # Loop over different depths d
			...
			lst_error.append([i, j, error])                          # Appending the error to the error_list
			print(f'For d={j} MSE={round(error,4)}')
		print('-------------------')
	lst_error = np.array(lst_error)
	print('END Q1_C\n')
